...
def iexit():
    res = messagebox.askokcancel('Confirm', 'Do you want to exit?')
    if res == True:
        root.destroy()
...
def search():
    ...
    if word in data:
        ...
        if res == True:
            ...
            textarea.delete(1.0, END)
            messagebox.showinfo('Information', "The Word Doesn't exist,please type a correct word")
            enterwordentry.delete(0, END)
    else:
...
root.title('Talking Dictionary Created by Ujjwal')
root.resizable(0, 0)
...
enterwordLabel = Label(root, text='Enter a Word', font=('casteller', 29, 'bold'), fg='red3', bg='whitesmoke')
